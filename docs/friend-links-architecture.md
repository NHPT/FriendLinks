...
管理员在插件中选择一个已发布的 Typecho 独立页面，或通过安装向导创建页面。
...
承载页必须使用主题的普通页面模板。安装向导应检测自定义模板：

- 新建页面时不指定自定义模板。
- 选择现有页面时，如果它仍使用 `page-links.php` 等友链模板，阻止保存并提示切换为普通页面模板。
- 管理员可明确确认后清空现有页面的自定义模板字段，以保留原 URL 并避免重复列表。
...
1. CLI Worker：推荐方案，由系统 Cron 调用。
2. 签名 HTTP Worker：无法使用 CLI Cron 时的兼容方案。
3. 访客触发的伪 Cron：不实现。

推荐命令形式：

```bash
php usr/plugins/FriendLinks/bin/console.php check --due --limit=50
```

插件后台展示可直接使用的 Cron 示例、最近心跳、最近成功运行时间和积压任务数量。
...
删除数据必须通过单独的“卸载并删除数据”操作完成，并要求管理员二次确认。
...
- 全局并发默认 5，单主机并发默认 1。
- 只对网络错误、408、425、429 和 5xx 执行有限重试，并对 `Retry-After` 设置上限。
...
9. 主要检测数据超过两倍 HTTP 检测周期仍未更新时，公开状态临时计算为 `unknown`，原结果只在管理端保留为“上次已知状态”。
...
HTTP Worker 只作为无法运行 CLI 的兼容入口，并要求 HTTPS。
入口只接受固定 Typecho 路由上的 `POST`。`bin/console.php` 必须检查 `PHP_SAPI`，拒绝通过 Web SAPI 执行；后台生成的 Cron 示例使用当前站点的绝对文件路径，不依赖 Cron 工作目录。
...
5. 插件是自动创建“友情链接”页面，还是只允许管理员选择现有页面；建议同时支持创建和选择。
